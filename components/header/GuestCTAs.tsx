...
export default function GuestCTAs({ onOpenLogin }: GuestCTAsProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      <button
        type="button"
        className="brut-btn-pill bg-sky/15 text-sky border-sky/30 px-4 py-2 text-sm flex items-center gap-2"
      >
        <Gift size={15} />
        免费得积分
      </button>
      <button
        type="button"
        onClick={() => onOpenLogin?.()}
        className="brut-btn-pill px-5 py-2 text-sm text-text-primary"
      >
        登录
      </button>
      <button
        type="button"
        onClick={() => onOpenLogin?.()}
        className="brut-btn-primary-pill px-5 py-2 text-sm"
      >
        开始创作
      </button>
    </div>
